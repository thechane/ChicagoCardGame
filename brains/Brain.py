...
class Brain(object):
    ...
    def ID_Hand(self,handStrings):
        ...
        if self.ofAKindResult[0][1] == 4 :
            ...
            return ('PAIR', 1000 + self.ofAKindResult[0][0], highCard, splitHand)       ##1000 + value of high card + card value of pair,value of suit
        else:
    ...
    def Score_Hand(self, hand, straightFlushValue, fourKindToZero, stats):
        ...
        for playerNum in hand:
            ...
            if (
                hand[playerNum]['handScore'][0] > bestScore[0] or
                (hand[playerNum]['handScore'][0] == bestScore[0] and
                    hand[playerNum]['handScore'][1] > bestScore[1]) or
                (hand[playerNum]['handScore'][0] == bestScore[0] and
                    hand[playerNum]['handScore'][1] == bestScore[1] and
                    hand[playerNum]['handScore'][2] > bestScore[2])
                ):
        ...
        return(self.Strip_Player(hand[self.winner]['name']) + ' wins_with ' + winningHand)
    ...
    def Poker_Discards(self,hand,posindex,handID):
        ...
        if handID[1] > 4000:   #Straight or better
            ...
            for scatterID in posindex:
                ...
                if (
                        (handID[1] < 3000 and                       ##two pair
                        (self.ofAKindResult[0][0] == cardID[0] or self.ofAKindResult[1][0] == cardID[0]))
                    or
                        (self.ofAKindResult[0][0] == cardID[0])     ##3 of a kind
                    or
                        cardID[0] > 12                              ##king or higher
                    ):
                    continue
    ...
    def Chicago_Question(self,configData,handData,playerNum):
        ...
        for suit in splitHand:
            ...
            Logger.info('... risk = ' + str(handData[playerNum]['risk']) + ', considering the ' + str(len(splitHand[suit])) + ' ' + str(splitHand[suit]))
            if (
                (len(handData) < 4 and len(splitHand[suit]) >= 4 and aceCount >= 2) or
                (len(handData) == 2 and len(splitHand[suit]) >= 4 and aceCount >= 2) or
                (len(handData) == 2 and aceCount > 2) or
                (len(handData) == 2 and aceCount == 2 and (kingCount > 1 or queenCount > 1))
                ):
                return True
            if (
                (len(handData) == 2 and len(splitHand[suit]) >= 3 and aceCount >= 2) or
                (len(handData) == 4 and len(splitHand[suit]) >= 4 and max(splitHand[suit]) > 10) or  #4 players, 4 or more cards of same suit and one of those cards > 10
                (len(handData) == 4 and len(splitHand[suit]) >= 3 and aceCount >= 2) or
                (len(handData) < 4 and aceCount > 2) or
                (len(handData) < 4 and aceCount >= 2 and (kingCount >= 2 or queenCount >= 2))
                ) and randint(0,9) < handData[playerNum]['risk']:                                  #players have risk factor set between 0 and 9.
                return True
        if highestCardValue < 11:
            return False
        if (
            (handID[1] > 1000 and handID[1] < 3000) and ##pair or 2 pair
            ...
            for cardName in handData[playerNum]['cardid']:
                cardID = self.ID_Card(cardName)
                if cardID[0] > 13 and cardID[0] != self.ofAKindResult[0][0] and len(handData) < 4 and randint(0,9) < handData[playerNum]['risk']:
                    return True
        if (
            (
                (handID[1] > 3000 and handID[1] < 4000) or  ##3 of a kind
                (handID[1] > 7000 and handID[1] < 8000)     ##4 of a kind
            ) and
            (
                (len(handData) == 2 and self.ofAKindResult[0][0] > 12) or
                (len(handData) == 3 and self.ofAKindResult[0][0] > 13) or
                (self.ofAKindResult[0][0] == 14)
            )
            ):
    ...
    def Return_Lowest_Card_With_Matching_Suit(self, hand, suit):
        ...
        for (index,cardname) in enumerate(hand):
            if cardname != 'DONE':
                ...
                if (
                        (cardID[1] == suit and suitMatch[0] is None) or
...
#todo - more intel here - keep one of each suit for example
    ...
    def Showdown_Turn_No_Chicago(self,hand,activeCard,inControl,pNum):
        Logger.info('BRAINS: fired Showdown_Turn_No_Chicago for player ' + str(pNum) + ' with hand ' + str(hand))
        ...
        for (index,cardname) in enumerate(hand):
            ...
            cardID = self.ID_Card(cardname)
            if (
                (activeCard is None or cardID[1] == activeCard[2]) and
                (activeCard is None or cardID[0] > activeCard[1]) and
                cardID[0] < lowWinningCardMatchingSuit[1]             ##we want to use the lowest card to break control
                ):
        ...
        if inControl is True or activeCard is None:
            ...
            if (doneCounter == 0 and lowCard[0] < 7) or (doneCounter == 1 and lowCard[0] < 5):
            ...
            #(index,value)
            lowcardSuited = self.Return_Lowest_Card_With_Matching_Suit(hand, activeCard[2])
            if doneCounter == 0 and lowcardSuited[0] is not None and lowcardSuited[1] < 5:
    ...
    def Human_Next_Turn(self, gsInst):
        ...
        def now_big_card(a,w):
            ...
            anim = Animation(x = gsInst.xpos_home[tmpTuple[0]],
                             y = 0,
                             d = duration,
                             t = 'in_out_quad')
            ...
            if not tmpList:
                def finshUp(an,wi):
                    ...
                    if gsInst.gameState['controlPlayer'] is not None and gsInst.gameState['chicago'] < 0:
                        gsInst.Display_Chicago_Buttons()
                    elif gsInst.gameState['chicago'] != -2 and gsInst.gameState['controlPlayer'] is None:
                    ...
                    if gsInst.gameState['discardFlag'] is True:
                        for sID in gsInst.hand[gsInst.currentPlayer]['posindex']:
                            if sID not in gsInst.discardNumber[gsInst.currentPlayer]:
                                gsInst.ids['card' + str(sID) + 'Image'].source = gsInst.cardFilePath + '/' + gsInst.hand[gsInst.currentPlayer]['cardid'][sID]
        ...
        def moveSmallCardsOffScreen(dt):
            ...
            if gsInst.manager is None:
                ...
                return True
            elif gsInst.holdIt is False and gsInst.dealingCards is False and gsInst.init is False:
                ...
                if all(item == 'DONE' for item in gsInst.hand[gsInst.currentPlayer]['cardid']):
                    gsInst.ids['endTurnButton'].disabled = False
                else:
                    for toXhomeIndex, scatterID in enumerate(gsInst.hand[gsInst.currentPlayer]['posindex']):
                        if gsInst.hand[gsInst.currentPlayer]['cardid'][scatterID] == 'DONE':
                            continue
                        tmpList.append( (toXhomeIndex,scatterID) )
                        smallCard = gsInst.ids['p' + str(gsInst.currentPlayer) + 'c' + str(scatterID) + 'Image']
                        anim = Animation(x = gsInst.xpos_center,
                ...
                Logger.info('waiting on self holdit, init and dealingSmallCards to release')
    ...
    def Next_Play(self, gsInst):
        ...
        gsInst.Save_Game()
        def _Next_Play():
            Logger.info('---------- _Next_Play fired for player ' + str(cPlayer) + ' ---------------, chicago gameState = ' + str(gsInst.gameState['chicago']))
            ...
            def Play_Card(scatterID):
                ...
                def smallcardGone(a,w):
                    ...
                    cImage.source = gsInst.cardFilePath + '/' + gsInst.hand[cPlayer]['cardid'][scatterID]
                    box = gsInst.ids['showDownLabel']
                    anim = Animation(x = box.pos[0] + (box.width / 2) - (gsInst.ids['card0'].width / 2),
                                     y = box.pos[1] + (box.height / 2) - (gsInst.ids['card0'].height / 2),
                                     d = duration,
                                     t='out_quint')
            ...
            if gsInst.gameState['controlPlayer'] is None:
                ...
                def Poker_Change_With_handID():
                    hID = self.Poker_Change(gsInst.hand[cPlayer]['cardid'],
                ...
                if gsInst.hand[cPlayer]['canDiscard'] is False:
                    ...
                    discardList = self.Poker_Discards(gsInst.hand[cPlayer]['cardid'],
                                                        gsInst.hand[cPlayer]['posindex'],
                                                        handID[0])
                else:
                    ##Let's try to position ourselves for the showdown / posible chicago call
                    discardList = self.Showdown_Discards(gsInst.hand[cPlayer]['cardid'],gsInst.hand[cPlayer]['posindex'])
                ...
                if not discardList:     ##If no discards
                    ...
                    return False
                #deal in new cards
                Pos = gsInst.Position_Small_Card_Offscreen(gsInst.ids['p1c1Image']) ##offscreen position same for all (dealer dependant)
                ...
                for counter, scatterID in enumerate(discardList):
                    ...
                    gsInst.Deal_Cards(cPlayer, gsInst.hand[cPlayer]['cardid'][scatterID])
                    scW = gsInst.ids['p' + str(cPlayer) + 'c' + str(scatterID) + 'Image']
                    duration = 1
                    if Configed_Bool("General", "fastPlay") is True:
                        duration = 0.5
                    anim = Animation(x = Pos[0], y = Pos[1], d = duration, t = 'in_out_quad')
                    anim = anim + Animation(x = gsInst.smallCardPos[cPlayer - 1][scatterID][0],
                                            y = gsInst.smallCardPos[cPlayer - 1][scatterID][1],
                                            d=0.5,
                                            t='in_out_quad')
                    #print str(counter) + ' ' + str(len(discardList))
                    if counter == len(discardList) - 1:
                        if Configed_Bool("General", "sound") is True and gsInst.init is False:
                            def dealSound(dt):
                    ...
                    anim.start(scW)
                return True
            elif gsInst.gameState['controlPlayer'] is not None and gsInst.gameState['activeCard'] is not None and all(item == 'DONE' for item in gsInst.hand[cPlayer]['cardid']):
                Logger.critical('Player num = ' + str(cPlayer) + '. This should never happen, we are continuing a game where the CPU has played but not completed end_turn yet.')
                return None
            elif gsInst.gameState['controlPlayer'] is not None and gsInst.gameState['chicago'] < 0:
                Logger.info('CPU Chicago Q fired')
                #shall we call Chicago?
                if self.Chicago_Question(gsInst.setConfig,gsInst.hand,cPlayer) is True:
                    Logger.info('CPU will play Chicago')
                    gsInst.Call_Chicago()
                    gsInst.Fun_Text(self.Strip_Player(gsInst.hand[cPlayer]['name']) + ' calls Chicago', None, 'yellow', 1.5)
                    Logger.info('CPU_Play requested from CPU_Play itself (CPU Chicago Q fired)')
                ...
                Play_Card(self.Showdown_Turn_Self_Chicago(gsInst.hand[cPlayer]['cardid']))
                ...
                scatterID = self.Showdown_Turn_Other_Chicago(gsInst.hand[cPlayer]['cardid'],
                ...
                if cPlayer == gsInst.gameState['controlPlayer']:
                    inControl = True
                scatterID = self.Showdown_Turn_No_Chicago(gsInst.hand[cPlayer]['cardid'],
...
###################################################################################################################
        def CPUwait(dt):
            ...
            returnValue = None
            if gsInst.holdIt is False and gsInst.init is False and gsInst.dealingCards is False:
