...
class GameScreen(Screen):
    ...
    fourOfaKindReset = BooleanProperty(False)
    ...
    def __init__(self, **kwargs):  # #Override Screen's constructor
        ...
        if kwargs.get('gameData') is None:
            ...
            self.current_player = kwargs.get('gameData')['current_player']
            ...
            Logger.info('Loaded in game data, current player now ' + str(self.current_player))
    ...
    def on_current_player(self, x, y):
        Logger.info('on_current_player fired:' + str(x) + ':' + str(y))
    ...
    def check_graphics(self):
        ...
        for pnum in self.hand:
            for index in range(5):
                self.ids[f'p{pnum}c{index}Image'].source = self.cardSmallBackImagePath
    ...
    def tutorial_text(self):
        ...
        if self.hand[self.current_player]['cpu'] is True:
            ...
            if self.hand[self.current_player]['can_discard'] is False:
                ...
                return ("Try to build a good showdown\n"
        ...
        elif self.gameState['controlPlayer'] is not None and self.gameState['chicago'] < 0:
            # Chicago Question
            if self.B.Chicago_Question(self.setConfig, self.hand, self.current_player) is True:
            ...
            in_control = False
            if self.current_player == self.gameState['controlPlayer']:
                in_control = True
            sid = self.B.Showdown_Turn_No_Chicago(self.hand[self.current_player]['cardid'],
                                                  self.gameState['activeCard'],
                                                  in_control,
                                                  self.current_player)
            return card_text(self.hand[self.current_player]['cardid'][sid])
    ...
    def show_tutorial(self, **kwargs):
        ...
        try:
            ...
            self.ids['tutorialLO2'].add_widget(self.ids['closeTutorialButtonPadding'])
            self.ids['tutorialLO'].add_widget(self.ids['svID'])
            self.ids['tutorialLO2'].add_widget(self.ids['closeTutorialButton'])
            self.ids['tutorialLO'].add_widget(self.ids['tutorialLabelPadding2'])
    ...
    def reset_game(self, kwargs):
        ...
        self.gameState = {
            ...
            'tmpActiveCard': None,
            # #Stores temp version of activeCard for transition between turns
    ...
    def on_size(self, screen, size):
        Logger.info(
            'on_size fired, ' + str(screen) + ',' + str(size) + ',' + str(self.width) + ',' + str(
                self.height))
        ...
        Logger.info('Screen Flags, bigScreen = ' + str(self.bigScreen) + ', smallScreen = ' + str(
            self.smallScreen))
        self.holdIt = True
    ...
    def Screen_Size_Calcs(self):
        ...
        try:
            for scatterID in self.hand[self.current_player]['posindex']:
                self.ids['card' + str(scatterID)].size_hint = self.xCardVector, self.yCardVector
            self.ids['activeCardImage'].size_hint = self.xCardVector, self.yCardVector
            self.ids['showDownLabel'].size_hint = self.xCardVector + 0.1, self.yCardVector + 0.055
        except:
            pass
        ...
        try:
            ...
            self.xpos_center = self.center_x - (self.width * self.xCardVector / 2)
            self.ypos_center = self.center_y - (self.height * self.yCardVector / 2)
        # #Card home pos
        ...
        try:
            ...
            for scatterID in self.hand[self.current_player]['posindex']:
                self.ids['card' + str(scatterID)].pos = int(self.xpos_center), 0 - self.ids[
                    'card0'].size[1] * 2
        except:
            return False
    ...
    def on_touch_down(self, touch):
        ...
        #             self.hand[self.current_player]['score'] += 10
        #             if Configed_Bool("General", "sound") is True:
        #                 self.cheat_sound.play()
            ...
            for scatterID in self.hand[self.current_player]['posindex']:
                if self.hand[self.current_player]['cardid'][scatterID] == 'DONE':
                    continue
            ...
            if someCardsFaceDown is True:
                for sID in self.hand[self.current_player]['posindex']:
                    ...
                    cImage.source = self.cardFilePath + '/' + \
                                    self.hand[self.current_player]['cardid'][sID]
            else:
            ...
            if self.gameState['controlPlayer'] is None:
                try:
                    self.Alarm_on_ID(self.B.ID_Hand(self.hand[self.current_player]['cardid']))
                except:
                    pass
            return False
        ...
        elif self.gameState['controlPlayer'] is not None and Get_Config_Bool(
                self.setConfig['viewDiscards']) is True:
    ...
    def Call_Chicago(self):
        self.gameState['chicago'] = copy(self.current_player)
        self.gameState['controlPlayer'] = copy(self.current_player)
        self.callChicago = []
        self.Update_Info(None)

    def Skip_Chicago(self):
        Logger.info('Skip_Chicago fired for player ' + str(self.current_player))
        if self.hand[self.current_player]['cpu'] is False and Configed_Bool("General",
                                                                           "sound") is True:
        ...
        if self.callChicago.count('No') == len(self.hand):
            ...
            newPlayer = Get_Next_Player(self.current_player, len(list(self.hand.keys())))
        self.Update_Info(None)
    ...
    def Display_Chicago_Buttons(self):
        ...
        nAnimIn = Animation(x=self.ids['mainFloat'].width + nb.width, y=nb.pos[1], d=0.3)
    ...
    def Move_All_Home(self, enableEndTurnFlag):
        ...
        if self.gameState['controlPlayer'] is not None and self.gameState['tmpActiveCard'] is None:
            self.ids['endTurnButton'].disabled = True
        if Configed_Bool("General", "sound") is True:
        ...
        def Move_Home(scatterID, toXhomeIndex, toYpos, speed, lastCard):
            ...
            def animation_complete(a, w):
                if enableEndTurnFlag is True and self.gameState['chicago'] > 0:
                    ...
                    if (
                            ...
                            self.hand[self.current_player]['cpu'] is False
                    ):
                        self.ids['endTurnButton'].disabled = False
            ...
            cardImage = self.ids['card' + str(scatterID) + 'Image']
            if cardImage.source != self.cardBackImagePath:
                cardImage.source = self.cardFilePath + '/' + \
                                   self.hand[self.current_player]['cardid'][scatterID]

        for cardxPos, scatterID in enumerate(self.hand[self.current_player]['posindex']):
            if (
                    self.hand[self.current_player]['cardid'][scatterID] == 'DONE' or
                    (
                ...
                continue
            lastCard = False
            if cardxPos == len(self.hand[self.current_player]['posindex']) - 1:
                lastCard = True
    ...
    def on_touch_up(self, touch):
        ...
        if self.ids['tutorialLO'].children:  # #if tutorial box is open
            ...
            return True
        try:
            if self.pressed_down[1] > self.ids['card0'].size[1] + 10 and self.gameState[
                'doubleTapConfirm'] is False:
                return True
        except:
            pass
            # to combat this:
        ...
        if self.gameState['controlPlayer'] is not None and self.gameState['chicago'] >= 0:
            ...
            if box.collide_point(*self.pressed_up) is True:
                ...
                for scatterID in self.hand[self.current_player]['posindex']:
                    ...
                    if card.collide_point(*self.pressed_up) is True:
                        cardID = self.B.ID_Card(self.hand[self.current_player]['cardid'][scatterID])
                        ...
                        if (
                                ...
                                self.B.Does_Player_Have_Matching_Suit(
                                    self.hand[self.current_player]['cardid'],
                ...
                if self.gameState['tmpActiveCard'] is not None:
                    for scatterID in self.hand[self.current_player]['posindex']:
                        card = self.ids['card' + str(scatterID)]
                        if card.collide_point(*self.pressed_up) is True and \
                                self.gameState['tmpActiveCard'][0] == scatterID:
                            self.gameState['tmpActiveCard'] = None
        ...
        for cardxPos, scatterID in enumerate(self.hand[self.current_player]['posindex']):
            ...
            if card.collide_point(
                    *self.pressed_up) is False:  # if last known up location does not collide with a card
                continue
            movedCard = True
            if (
                    (card.pos[0] > int(self.right - self.card_xsize_rightvector) and card.pos[
                        1] > self.card_ysize_vector) or
                    (card.pos[0] < 0 - self.card_xsize_leftvector and card.pos[
                        1] > self.card_ysize_vector) or
                ...
                if self.gameState['controlPlayer'] is None:
                    ...
                    self.flash_box("No exchanging of cards\nduring the showdown")
        ...
        Logger.info('CURRENTPLAYER = ' + str(self.current_player))
        ...
        Logger.info('tmpControlPlayer = ' + str(self.gameState['tmpControlPlayer']))
    ...
    def Reorder_Hand(self, cardStartPos, scatterID, xpos):
        Logger.info(
            'Reorder_Hand Fired, Pos Index:' + str(self.hand[self.current_player]['posindex']))
        newPosIndex = []
        for index in range(0, self.setConfig['handCount']):
        ...
        if xpos < self.xpos_home[cardStartPos]:
            ...
            for cardPos in range(0, self.setConfig['handCount']):
                ...
                if newPosIndex[cardPos] != scatterID and xpos < self.xpos_home[cardPos]:
                    madeSwitch = True
                    Logger.info('madeSwitch (positions):' + str(cardStartPos) + '->' + str(cardPos))
                    newPosIndex[cardPos] = scatterID
                    break
                else:
                    newPosIndex[cardPos] = copy(self.hand[self.current_player]['posindex'][cardPos])
            for index in range(counter, cardStartPos + 1):
                newPosIndex[index] = copy(self.hand[self.current_player]['posindex'][index - 1])
            for index in range(cardStartPos + 1, self.setConfig['handCount']):
                newPosIndex[index] = copy(self.hand[self.current_player]['posindex'][index])
        else:
            ...
            for index in range(cardStartPos - 1, -1, -1):
                newPosIndex[index] = copy(self.hand[self.current_player]['posindex'][index])
        if madeSwitch:
            self.hand[self.current_player]['posindex'] = newPosIndex
        return madeSwitch

    def Deal_Cards(self, playerNumber, *discarded):
        Logger.info(
            'Deal_Cards Fired, player num:' + str(playerNumber) + ', discards = ' + str(discarded))
        self.dealingCards = True
        ...
        if len(self.gameState['deck']) < len(
            ...
            newCards = newCards + random.sample(sorted(self.gameState['deck']),
                                                len(discarded) - len(newCards))
        else:
            newCards = random.sample(sorted(self.gameState['deck']), len(discarded))
        self.gameState['deck'] = self.gameState['deck'] - set(newCards)
        if discarded[
            0] is not None:  # #Since none will only be possible is all discards are None (new game)
    ...
    def Center_Pos(self, cardNumber):
        ...
        anim = Animation(x=self.xpos_center + cardNumber * 20, y=self.ypos_center, d=0.2)
        anim.start(card)
    ...
    def Discard_Card(self, scatterID):
        ...
        if self.gameState['doubleTapConfirm'] is True and self.gameState['discardFlag'] is True:
        ...
        if len(self.discardNumber[self.current_player]) >= 5:
            self.flash_box("You can not discard the\nsame card twice in one round")
            self.Center_Pos(scatterID)
            return
        elif scatterID in self.discardNumber[self.current_player]:
        ...
        def dealIt(dt):
            self.discardNumber[self.current_player].append(scatterID)
            self.Deal_Cards(self.current_player, self.hand[self.current_player]['cardid'][scatterID])

        Clock.schedule_once(dealIt, 0.3)

    def Alarm_on_ID(self, result):
        Logger.info('Alarm_on_ID fired')
        if result[1] > 1000 and self.hand[self.current_player]['hand'] != result[0]:
            self.Fun_Text(result[0], self.current_player, 'yellow', 1)
            self.hand[self.current_player]['hand'] = result[0]
        ...
        with Particle(size_hint_x=0.01, width=self.ids['infoFloat'].width * 0.01) as P:
            ...
            if player is not None and Configed_Bool("General", "effects") is True:
                ...
                if effect is not None:
                    ...
                    P.show(id=effect, x=xpos, y=ypos, layout=self.ids['infoFloat'])
            ...
            if Configed_Bool("General", "simplefuntext") is True:
                ...
                if self.smallScreen is True and Metrics.dpi_rounded < 320:
                    color = color + '25pt/'
                        ...
                        Metrics.dpi_rounded >= 240 and self.smallScreen is False) or self.bigScreen is True:
                    color = color + '150pt/'
                else:
                    color = color + '50pt/'
            else:
            ...
            for doneCount, char in enumerate(text):
                ...
                if char == ' ':
                    ...
                    l.source = './images/funtext/funtext' + color + char.lower() + '.png'
                    ...
                    anim = anim + Animation(x=self.ids['mainFloat'].width + l.width, y=ypos + 10,
                                            t='in_expo')
                    anim.bind(on_complete=animation_complete)
                    anim.start(l)
    ...
    def Update_Info(self, infoString):
        ...
        if infoString is None:
            iString = padding + "[size=" + str(size) + "sp][color=0000ff]\n Scorecard\n[/color]"
            iString += "[color=0000ff][/color]"
            for p in self.hand:
                iString += padding + "[color=0000ee] "
                iString += self.hand[p]['name'][0:7] + ' : ' + str(self.hand[p]['score'])
            ...
            if self.gameState['controlPlayer'] is None:
                ...
                if self.portrait is True and self.smallScreen is True:
                    iString += padding + "[b]Pkr Rnd: " + str(self.gameState['roundNumber']) + "/"
                else:
                    iString += padding + " [b]Poker Round: " + str(
                        self.gameState['roundNumber']) + " of "
                ...
                # #Showdown
                if self.portrait is True and self.smallScreen is True:
                    iString += padding + "Ctl:" + self.hand[self.gameState['controlPlayer']][
                                                      'name'][0:7]
                else:
                    iString += padding + " Control:" + self.hand[self.gameState['controlPlayer']][
                                                           'name'][0:7]
    ...
    def Display_Showdown_Cards(self, pNum, allFlag):
        ...
        for doneCount, cardName in enumerate(cardList):
            ...
            anim = Animation(x=xpos,
                             y=self.ypos_center,
                             t='in_quad')
            anim = anim + Animation(x=xpos,
                                    y=yPos,
                                    d=2.5)
            anim = anim + Animation(x=c.width + mainFloat.width,
                                    y=yPos,
                                    t='in_expo')
            anim.bind(on_complete=animation_complete)
            anim.start(c)
    ...
    def Update_Player_Circle(self):
        ...
        for player in self.hand:
            ...
            if self.gameState['controlPlayer'] is not None and player == self.gameState[
                'controlPlayer']:
    ...
    def End_Turn(self, **kwargs):
        newPlayer = copy(self.current_player)
        ...
        if self.gameState['roundNumber'] == 1 and set(
                self.hand[self.current_player]['cardid']) == set([None, None, None, None, None]):
            return True

        # #Prevent players from not entering a card during showdown
        if (
                self.gameState['controlPlayer'] is not None and
                self.gameState['controlPlayer'] == self.current_player and
        ...
        Logger.info(
            '---------------ENDING TURN for player ' + str(self.current_player) + '----------------')
        if self.hand[self.current_player]['cpu'] is False and Configed_Bool("General",
                                                                           "sound") is True:
            self.dealCard_sound.play()
        ...
        Logger.info('activeCard = ' + str(self.gameState['activeCard']))
        Logger.info('tmpActiveCard = ' + str(self.gameState['tmpActiveCard']))
        Logger.info('poker round count = ' + str(self.setConfig['pokerRoundCount']))
        ...
        if self.gameState['activeCard'] is None and self.gameState['controlPlayer'] is not None:
        ...
        if (
            ...
            faceUp = (self.gameState['tmpActiveCard'][0], copy(
                self.hand[self.current_player]['cardid'][self.gameState['tmpActiveCard'][0]]))
            self.gameState['tmpActiveCard'] = None
            self.ids['activeCardImage'].color = [1, 1, 1, 1]
            self.ids['activeCardImage'].source = self.cardFilePath + '/' + \
                                                 self.hand[self.current_player]['cardid'][
                                                     self.gameState['activeCard'][0]]
            self.gameState['tmpControlPlayer'] = copy(self.current_player)
            self.hand[self.current_player]['showDownDiscards'].append(
                copy(self.hand[self.current_player]['cardid'][self.gameState['activeCard'][0]]))
            self.hand[self.current_player]['cardid'][self.gameState['activeCard'][0]] = 'DONE'
        ...
        # #card has been played but since above not true it is a discard in Chicago round
            ...
            if self.gameState['tmpActiveCard'][2] != self.gameState['activeCard'][
                2]:  # #suits do not match
                self.B.playerCantPlaySuit[self.current_player - 1].append(
                    self.gameState['activeCard'][2])
            Logger.info('player ' + str(self.current_player) + ' does NOT take control')
            faceUp = (self.gameState['tmpActiveCard'][0],
                      copy(self.hand[self.current_player]['cardid'][
            ...
            dACanim.start(card)
            self.hand[self.current_player]['showDownDiscards'].append(
                copy(self.hand[self.current_player]['cardid'][self.gameState['tmpActiveCard'][0]]))
            self.hand[self.current_player]['cardid'][self.gameState['tmpActiveCard'][0]] = 'DONE'
            self.gameState['tmpActiveCard'] = None
        elif self.gameState['controlPlayer'] is None:
        ...
        showdownRoundCompleteFlag = None  # None = middle of showdown, >0 means end showdown round, 0 = start of showdown
        if (
                showdownCompleteFlag is False and
                newShowdownRound is False and
                self.gameState['controlPlayer'] is not None and
        ...
        if self.gameState['controlPlayer'] is not None:
            for pNum in self.hand:
                Logger.info('PlayerNum ' + str(pNum) + ' hand = ' + str(self.hand[pNum]['cardid']))
                if showdownRoundCompleteFlag is None:
                    showdownRoundCompleteFlag = self.hand[pNum]['cardid'].count('DONE')
                elif showdownRoundCompleteFlag != self.hand[pNum]['cardid'].count('DONE'):
                    showdownRoundCompleteFlag = None
                    break
            if showdownRoundCompleteFlag == 5:
                showdownCompleteFlag = True
        Logger.info('showdownCompleteFlag = ' + str(showdownCompleteFlag))
        Logger.info('showdownRoundCompleteFlag = ' + str(showdownRoundCompleteFlag))
        ...
        if showdownCompleteFlag is True:
            ...
            if self.gameState['chicago'] > 0:
                ...
                if self.gameState['tmpControlPlayer'] != self.gameState['controlPlayer']:
                    ...
                    if Get_Config_Bool(self.setConfig['chicagoTwo']) is True and \
                            self.gameState['activeCard'][1] == 2:
                ...
                if Get_Config_Bool(self.setConfig['pokerAfterShowdownScoring']) is True and \
                        self.hand[self.current_player]['score'] < 52:
            ...
            def Now_Small_Cards(a, w):
                ...
                def done_animation(anim, widget):
                    ...
                    c = self.ids['card0']
                    self.ids['activeCardImage'].pos = sdl.pos[0] + (sdl.width / 2) - (c.width / 2), \
            ...
            for index in range(0, self.setConfig['handCount']):
                ...
                doneAnim.start(card)
            acAnim = Animation(x=int(self.xpos_center), y=0 - yOffset, d=duration)
            ...
            if self.gameState['roundNumber'] >= self.setConfig[
                'pokerRoundCount'] and self.current_player == self.dealerPlayer:
                Logger.info(
                    'Poker rounds completed for this cycle - setting control player to ' + str(
                        self.B.winner))
                ...
                newPlayer = Get_Next_Player(self.current_player, len(self.hand))
            Logger.info('player now = ' + str(self.current_player))

            # #Update current players (poker) roundCount
            self.hand[self.current_player]['roundCount'] = self.gameState['roundNumber']
            ...
            if self.gameState['controlPlayer'] is None and counter == len(self.hand):
                ...
                Logger.info(
                    'roundNumber (poker) incremented to ' + str(self.gameState['roundNumber']))
                for pNum in self.discardNumber:
                    self.discardNumber[pNum] = []

            # #show tutorial is needed
            if (
                    self.hand[self.current_player]['cpu'] is False and
            ...
            if (
                    ...
                    self.dealerPlayer == self.current_player
            ):
                if Configed_Bool("General", "tutorial") is True:
            ...
            if showdownRoundCompleteFlag is not None and showdownRoundCompleteFlag > 0:
                ...
                if self.gameState['tmpControlPlayer'] is not None:
                    Logger.info('control has switched from ' + str(
                        self.gameState['controlPlayer']) + ' to ' + str(
    ...
    def Anim_Cards_End_Turn(self, faceUp, newPlayer):
        ...
        Logger.info('Anim_Cards_End_Turn Fired, newPlayer = ' + str(
            newPlayer) + ', current player is ' + str(self.current_player))
        ...
        Logger.info('lastScatter = ' + str(lastScatter) + ', tmpList = ' + str(tmpList))

        ##copy the current_player so when we return after changig it, we continue with original value
        cPlayer = copy(self.current_player)

        def nextTurn(a, w):
            Logger.info('Anim_Cards_End_Turn completed')
            if newPlayer is not None and self.current_player != newPlayer:
        ...
        if not tmpList:
            Logger.info('Current Player ' + str(cPlayer) + ' has all DONE cards')
            nextTurn(None, None)
            return False
        ...
        def return_smallcards(a, w):
            Logger.info(
                'return_smallcards fired wth w.sID = ' + str(w.sID) + ' and lastScatter = ' + str(
                    lastScatter))
        ...
        def no_newplayer(a, w):
            ...
            self.Move_All_Home(False)
            if self.hand[cPlayer]['cpu'] is False and self.gameState[
                'controlPlayer'] is not None and self.gameState['chicago'] < 0:
        ...
        for index, scatterID in enumerate(self.hand[cPlayer]['posindex']):
            ...
            if faceUp[0] is not None and scatterID == faceUp[0]:
                ...
                animDiscard = Animation(x=self.discardCardPos[cPlayer - 1][dStackPos][0],
                                        y=self.discardCardPos[cPlayer - 1][dStackPos][1],
                                        t='in_out_quad')
                # small dis card :)
                dCard = self.ids['p' + str(cPlayer) + 'c' + str(dStackPos) + 'DiscardImage']
                _popToTop(dCard)
            ...
            if self.hand[cPlayer]['cpu'] is True and cPlayer != newPlayer:
                ...
                if newPlayer != cPlayer:
                    anim = anim + Animation(x=int(self.xpos_center), y=y, d=0.3)
                    anim.bind(on_complete=return_smallcards)
                elif index == len(self.hand[cPlayer]['cardid']) - self.hand[cPlayer][
                    'cardid'].count('DONE') - 1:
        ...
        if Get_Config_Bool(self.setConfig['viewDiscards']) is True:
            for index in range(0, len(self.hand[cPlayer]['posindex'])):
                _popToTop(self.ids['p' + str(cPlayer) + 'c' + str(index) + 'DiscardImage'])
