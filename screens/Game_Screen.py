...
class Game_Screen(Screen):
    ...
    def Tutorial_Text(self):
        ...
        if self.hand[self.currentPlayer]['cpu'] is True:
            ...
            # #Chicago Question
            if self.B.Chicago_Question(self.setConfig, self.hand, self.currentPlayer) is True:
            ...
            # #Chicago
            sID = self.B.Showdown_Turn_Self_Chicago(self.hand[self.currentPlayer]['cardid'])
            return cardText(self.hand[self.currentPlayer]['cardid'][sID])
        elif self.gameState['chicago'] > 0:
            # #Other Chicago
            sID = self.B.Showdown_Turn_Other_Chicago(self.hand[self.currentPlayer]['cardid'],
    ...
    def Reset_Game(self, kwargs):
        ...
        for index in range(1, playerCount):
            ...
            self.hand[index] = {'cardid':copy(tmpHand),
                                ...
                                'risk': random.randint(0, 9),  # #0 takes little to no risks, 9 takes big risks
                                'name': name}
    ...
    def on_size(self, screen, size):
        ...
        Logger.info('Screen Flags, bigScreen = ' + str(self.bigScreen) + ', smallScreen = ' + str(self.smallScreen))
        self.holdIt = True
...
#                 self.cheat_sound.play()
            ...
            if someCardsFaceDown is True:
                for sID in self.hand[self.currentPlayer]['posindex']:
                    ...
                    cImage.source = self.cardFilePath + '/' + self.hand[self.currentPlayer]['cardid'][sID]
            else:
            ...
            if self.gameState['controlPlayer'] is None:
                try:
                    self.Alarm_on_ID(self.B.ID_Hand(self.hand[self.currentPlayer]['cardid']))
                except:
                    pass
            return False
...
# todo This routine is a bit crap - should improve
    ...
    def Skip_Chicago(self):
        ...
        if self.hand[self.currentPlayer]['cpu'] is False and Configed_Bool("General", "sound") is True:
    ...
    def Move_All_Home(self, enableEndTurnFlag):
        ...
        def Move_Home(scatterID, toXhomeIndex, toYpos, speed, lastCard):
            ...
            cardImage = self.ids['card' + str(scatterID) + 'Image']
            if cardImage.source != self.cardBackImagePath:
                cardImage.source = self.cardFilePath + '/' + self.hand[self.currentPlayer]['cardid'][scatterID]
    ...
    def on_touch_up(self, touch):
        ...
        if self.ids['tutorialLO'].children:  # #if tutorial box is open
            ...
            return True
        try:
            if self.pressed_down[1] > self.ids['card0'].size[1] + 10 and self.gameState['doubleTapConfirm'] is False:
                return True
        except:
            pass
            # to combat this:
    ...
    def Reorder_Hand(self, cardStartPos, scatterID, xpos):
        Logger.info('Reorder_Hand Fired, Pos Index:' + str(self.hand[self.currentPlayer]['posindex']))
        newPosIndex = []
        for index in range(0, self.setConfig['handCount']):
    ...
    def Deal_Cards(self, playerNumber, *discarded):
        Logger.info('Deal_Cards Fired, player num:' + str(playerNumber) + ', discards = ' + str(discarded))
        self.dealingCards = True
        ...
        if len(self.gameState['deck']) < len(discarded):  # #If deck is running too low and we have more discarded cards than we can replace
            ...
            newCards = newCards + random.sample(sorted(self.gameState['deck']), len(discarded) - len(newCards))
        else:
        ...
        with Particle(size_hint_x=0.01 , width=self.ids['infoFloat'].width * 0.01) as P:
            ...
            if Configed_Bool("General", "simplefuntext") is True:
                ...
                if self.smallScreen is True and Metrics.dpi_rounded < 320:
                    color = color + '25pt/'
                elif (Metrics.dpi_rounded >= 240 and self.smallScreen is False) or self.bigScreen is True:
                    color = color + '150pt/'
                else:
                    color = color + '50pt/'
            else:
            ...
            for doneCount, char in enumerate(text):
                ...
                if char == ' ':
                    ...
                    anim = anim + Animation(x=self.ids['mainFloat'].width + l.width, y=ypos + 10, t='in_expo')
                    anim.bind(on_complete=animation_complete)
                    anim.start(l)
        ...
        if infoString is None:
            ...
            if self.gameState['controlPlayer'] is None:
                ...
                if self.portrait is True and self.smallScreen is True:
                    ...
                    iString += padding + " [b]Poker Round: " + str(self.gameState['roundNumber']) + " of "
                ...
                # #Showdown
                if self.portrait is True and self.smallScreen is True:
                    iString += padding + "Ctl:" + self.hand[self.gameState['controlPlayer']]['name'][0:7]
                else:
    ...
    def Display_Showdown_Cards(self, pNum, allFlag):
        ...
        for doneCount, cardName in enumerate(cardList):
            ...
            xpos = self.xpos_center + (c.width * 0.25) * (len(self.hand[pNum]['showDownDiscards']) - doneCount)
    ...
    def Update_Player_Circle(self):
        ...
        for player in self.hand:
            ...
            if self.gameState['controlPlayer'] is not None and player == self.gameState['controlPlayer']:
    ...
    def End_Turn(self, **kwargs):
        ...
        Logger.info('---------------ENDING TURN for player ' + str(self.currentPlayer) + '----------------')
        if self.hand[self.currentPlayer]['cpu'] is False and Configed_Bool("General", "sound") is True:
            self.dealCard_sound.play()
        ...
        if (
            ...
            if self.gameState['tmpActiveCard'][2] != self.gameState['activeCard'][2]:  # #suits do not match
                self.B.playerCantPlaySuit[self.currentPlayer - 1].append(self.gameState['activeCard'][2])
            Logger.info('player ' + str(self.currentPlayer) + ' does NOT take control')
        ...
        if showdownCompleteFlag is True:
            ...
            if self.gameState['chicago'] > 0:
                ...
                if self.gameState['tmpControlPlayer'] != self.gameState['controlPlayer']:
                    ...
                    if Get_Config_Bool(self.setConfig['chicagoTwo']) is True and self.gameState['activeCard'][1] == 2:
            ...
            if self.gameState['controlPlayer'] is None and counter == len(self.hand):
                ...
                Logger.info('roundNumber (poker) incremented to ' + str(self.gameState['roundNumber']))
    ...
    def Anim_Cards_End_Turn(self, faceUp, newPlayer):
        ...
        def return_smallcards(a, w):
            Logger.info('return_smallcards fired wth w.sID = ' + str(w.sID) + ' and lastScatter = ' + str(lastScatter))
        ...
        for index, scatterID in enumerate(self.hand[cPlayer]['posindex']):
            ...
            if self.hand[cPlayer]['cpu'] is True and cPlayer != newPlayer:
                ...
                elif index == len(self.hand[cPlayer]['cardid']) - self.hand[cPlayer]['cardid'].count('DONE') - 1:
