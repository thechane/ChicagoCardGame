...
class BuyButton(Button):
    ...
    def _on_purchase(self, checkFlag):
        ...
        for widget in self.parent.walk():
            ...
            if (widget.id is not None) and ('flaggedcards' in widget.id or 'noads' in widget.id):
        ...
        if self.active is True:
            ...
            if self.name != 'noads_switch':
                for widget in self.parent.walk():
                    ...
                    if (widget.id is not None) and ('flaggedcards' in widget.id and self.name != widget.id):
                        widget.active = False
        elif int(App.get_running_app().config.getdefault("General", "sound", True)) == 1:
...
class Shop_Screen(Screen):
    ...
    def __init__(self, **kwargs):                               ##Override Screen's constructor
        ...
        super(Shop_Screen, self).__init__(**kwargs)             ##but also run parent class constructor (__init__)
        innerShopGL = self.ids['innerShopGL']
        if platform == 'android':
            for fullsku in App.get_running_app().billing.skus:
                name = fullsku.split('.')[-1]
                innerShopGL.add_widget( BuyButton(sku=fullsku,size_hint_x=0.8,text=name.split('_')[0] + ' flag cards') )
                innerShopGL.add_widget( Label(size_hint_x=0.1) )
                innerShopGL.add_widget( ToggleButton(name=name+'_switch',disabled=True,size_hint_x=0.2) )
...
#         popup.open()
        ...
        for widget in self.walk():
            ...
            if widget.id is not None and 'flaggedcards_buyButton' in widget.id and widget.text[-6:] != 'active':
                tmp = widget.id.split('_')[0]
                widget.prettytext = tmp + ' card design'
                widget.text = widget.prettytext
            elif widget.id is not None and'noads_buyButton' in widget.id and widget.text[-6:] != 'active':
    ...
    def on_leave(self):
        ...
        for widget in self.walk():
            #print("{} -> {}".format(widget, widget.id))
            if widget.id is not None and 'flaggedcards_switch' in widget.id and widget.active is True:
